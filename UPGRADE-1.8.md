...
- Be careful when trying to create indexes in the migrations. `Doctrine\DBAL\Schema\Table` - throws exception for already existing indexes, before - returns existing index.

- Added json type supports for PostgreSQL >=9.2. So if you use this data type before (it stored json data in database like text), we recommended to create migration and convert your old data from text to json, f.e see `Oro\Bundle\ReportBundle\Migrations\Schema\v1_4\UpdateReportJsonArrayQuery`.
Do not foget construction `USING <column>::JSON`, it helps convert your text data with valid json data to json.

To be able to equals json data we recommended to use function `CAST` - convert json data to the text:
```php
...
- We disabled possibility to automatically renaming autogenerated indexes.
It is a very heavy operation for existing database and require a lot of effort from developers to prepare proper migrations as well. See `Oro/Bundle/MigrationBundle/Migration/Schema/SchemaDiff.php`.

#####Start from doctrine/orm version 2.5.0

- When using `Doctrine\ORM\QueryBuilder` it needs to specify the table name and column
in the conditions. We recommended to check conditions in the custom queries.
...
In the native sql queries tests we recommended to check and correct columns aliases.
```sql
-- Before
