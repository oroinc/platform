...
- Class `Oro\Bundle\SalesBundle\Dashboard\Provider\OpportunityByStatusProvider`
    ...
    - removed property `protected $aclHelper`
    - removed property `protected $ownerHelper`
...
- Class `Oro\Bundle\SalesBundle\Entity\Repository\OpportunityRepository`
    ...
    - changed signature of method `getNewOpportunitiesAmount`
        - removed parameter `$aclHelper`
        - removed parameter `$owners`
        - added parameter `WidgetProviderFilterManager $widgetProviderFilter`
        - added parameter `WidgetOptionBag $widgetOptions`
    - changed signature of method `getWonOpportunitiesToDateCount`
        - removed parameter `$aclHelper`
        - removed parameter `$owners`
        - added parameter `WidgetProviderFilterManager $widgetProviderFilter`
        - added parameter `WidgetOptionBag $widgetOptions`
- Class `Oro\Bundle\SalesBundle\Provider\B2bBigNumberProvider`
    - construction signature of was changed. The parameter `AclHelper $aclHelper` was replaced by `WidgetProviderFilterManager $widgetProviderFilter`
    - removed property `protected $aclHelper`
    - added property `protected $widgetProviderFilter`
...
- Class `Oro\Bundle\SalesBundle\Provider\Opportunity\ForecastProvider`
    - construction signature was changed. The parameter `AclHelper $aclHelper` was replaced by `WidgetProviderFilterManager $widgetProviderFilter`
    - removed property `protected $aclHelper`
    - added property `protected $widgetProviderFilter`
