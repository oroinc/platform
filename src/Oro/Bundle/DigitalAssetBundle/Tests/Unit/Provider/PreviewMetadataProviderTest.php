...
class PreviewMetadataProviderTest extends TestCase
{
    ...
    public function testGetMetadataWhenNotImage(): void
    {
        ...
        $this->mimeTypeChecker->expects($this->once())
            ->method('isImageMimeType')
            ->with(self::MIME_TYPE_1)
            ->willReturn(false);

        $this->fileIconProvider->expects($this->once())
        ...
        $this->fileTitleProvider->expects($this->once())
            ->method('getTitle')
            ->with($file)
