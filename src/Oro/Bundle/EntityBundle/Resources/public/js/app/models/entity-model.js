...
import entitySerializer from 'oroentity/js/app/models/entity-serializer';
...
const EntityModel = BaseModel.extend(_.extend({}, Chaplin.SyncMachine, /** @lends EntityModel.prototype */ {
    ...
    constructor: function EntityModel(data, options) {
        ...
        this.registry = EntityModel.registry;
        this._relationships = {};
    ...
    registryRelieve: function(owner) {
        this.registry.relieve(this, owner);
    },
    ...
    _initRelationships: function(attrs) {
        ...
        _.each(attrs, function(value, name) {
            ...
            if (Array.isArray(data)) {
                ...
                relation = EntityModel._getRelationshipCollection(params, this);
            ...
            if (this._relationships[name] !== relation) {
                if (this._relationships[name]) {
                    this.registry.relieve(this._relationships[name], this);
                }
    ...
    getRelationship: function(name, applicant) {
        ...
        if (relation) {
            this.registry.retain(relation, applicant);
        }
        return relation;
    },
    ...
    getEntityModel: function(params, applicant) {
        ...
        let model = this.registry.fetch(globalId, applicant);
        ...
        if (!model) {
            ...
            try {
                this.registry.put(model, applicant);
            } catch (error) {
                /*
                ...
                if (/is already registered/.test(error.message) && rawData) {
                    ...
                    model = this.registry.fetch(globalId, applicant);
...
EntityModel._getRelationshipCollection = function(params, applicant) {
    return mediator.execute('getEntityRelationshipCollection', params, applicant);
};
