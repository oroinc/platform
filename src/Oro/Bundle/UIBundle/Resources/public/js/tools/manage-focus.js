...
export default {
     ...
     * Return first element in radio group by [name]
     * @param {HTMLElement[]} elements
     * @returns {(HTMLElement|Array)}
     */
    filterFirstInRadioGroup(elements) {
        return elements.reduce((collection, element) => {
            if (element.type !== 'radio') {
                collection.push(element);
                return collection;
            }

            if (!collection.find(node => node.name === element.name)) {
                collection.push(element);
            }
    ...
    preventTabOutOfContainer(e, container) {
        ...
        const tabbableElements = this.filterFirstInRadioGroup($container.find(':visible:tabbable').toArray());
