...
import template from 'tpl-loader!oroui/templates/message-item.html';
import MultiUseResourceManager from 'oroui/js/tools/multi-use-resource-manager';
import cyrb53 from 'oroui/js/tools/cyrb53';
...
const resolveContainer = function(options) {
    if ($(options.container).is(config.container) && $(config.temporaryContainer).length) {
        return config.temporaryContainer;
    }
...
const messenger = {
    ...
    notificationMessage: function(type, message, options = {}) {
        const container = options.container || config.container;
        ...
        if (afterReload && window.localStorage) {
            afterReloadQueue = JSON.parse(localStorage.getItem(config.storageKey) || '[]');
            afterReloadQueue.push([type, message, options]);
            localStorage.setItem(config.storageKey, JSON.stringify(afterReloadQueue));
    ...
    setup: function(options) {
        ...
        $(document).on('remove', config.temporaryContainer, this.removeTemporaryContainer);
    },

    flushStoredMessages: function() {
        if (window.localStorage) {
            queue = queue.concat(JSON.parse(localStorage.getItem(config.storageKey) || '[]'));
    ...
    clear: function(namespace, options = {}) {
        const opt = _.extend({}, config, options);
        const selectors = [];
        if (namespace) {
