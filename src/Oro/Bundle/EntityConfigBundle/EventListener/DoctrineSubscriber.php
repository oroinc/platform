...
class DoctrineSubscriber implements EventSubscriber
{
    ...
    public function loadClassMetadata(LoadClassMetadataEventArgs $event)
    {
        ...
        $rels      = $metadata->getAssociationMappings();

        $this->metadataCache[$className] = $metadata;
        $allMetadata = $em->getMetadataFactory()->getAllMetadata();

        // remember many-to-one rels and propagate them to other side of relation
        foreach ($rels as $mappingName => $mapping) {
            $relations[$mapping['targetEntity']] = '';

            // get targetEntity metadata
            $targetEntityMetadata = $em->getClassMetadata($mapping['targetEntity']);

            // add relation to $className entity
