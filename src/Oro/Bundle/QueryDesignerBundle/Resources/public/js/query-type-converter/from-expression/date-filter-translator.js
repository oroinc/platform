import {compact, findKey, findWhere, isEmpty, pick, unique, where} from 'underscore';
...
import {BinaryNode, ConstantNode, FunctionNode, GetAttrNode}
...
class DateFilterTranslatorFromExpression extends AbstractFilterTranslator {
    ...
    resolveOperatorParams(node) {
        ...
        const operatorParams = matchedParams.find(params => {
            ...
            if (
                ...
                Object.assign(params, pick(partParams, 'part'));
                ...
                Object.assign(params, pick(partParams, 'part'), valueParams);
            } else {
                return false;
            }
    ...
    resolvePartParams(node) {
        ...
        if (node instanceof FunctionNode) {
            partParams = findWhere(this.constructor.PART_MAP, {propModifier: node.attrs.name});
    ...
    resolveValueParams(node, partParams) {
        ...
        if ( // if value a constant code
            ...
            (variable = findKey(partParams.variables, value => value === node.attrs.name)) !== void 0
        ) {
    ...
    checkOperation(filterConfig, operatorParams) {
        const variables = compact(unique([
            operatorParams,
            operatorParams.left,
            operatorParams.right
        ].map(param => param && param.variable)));
        ...
        return super.checkOperation(filterConfig, operatorParams) &&
            ...
            variables.every(variable => !isEmpty(externalWidgetOptions.dateVars[operatorParams.part][variable]));
    }
}
