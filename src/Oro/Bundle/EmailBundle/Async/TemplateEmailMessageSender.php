...
class TemplateEmailMessageSender
{
    ...
    public function sendTranslatedMessage(array $message, array &$failedRecipients = []): int
    {
        ...
        return $this->templateEmailManager
            ->sendTemplateEmail(
                From::emailAddress($message['fromEmail'], $message['fromName']),
                [$this->getRecipient($message)],
                new EmailTemplateCriteria($message['template']),
                $message['body'],
                $failedRecipients
            );
    }
    ...
    {
        if (!isset($message['template'], $message['fromEmail'], $message['fromName'], $message['body'])) {
            return false;
        }
