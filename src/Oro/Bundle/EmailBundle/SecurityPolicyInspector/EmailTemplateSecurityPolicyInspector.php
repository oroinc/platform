...
class EmailTemplateSecurityPolicyInspector implements LoggerAwareInterface
{
    ...
    public function inspectByName(
        ...
        if (!$templateName instanceof EmailTemplateCriteria) {
            $templateName = new EmailTemplateCriteria($templateName);
        }

        try {
            /** @var EmailTemplate $emailTemplate */
            $template = $emailTemplateRepository->findWithLocalizations($templateName);
        } catch (NonUniqueResultException | NoResultException $exception) {
            $this->logger->warning(
                'Email template "{name}" not found, skipping security policy inspection.',
                ['name' => $templateName, 'exception' => $exception]
            );
