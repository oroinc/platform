...
class EmailModelFromEmailTemplateFactoryTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->factory->setEmailAttachmentModelFactory($this->emailAttachmentModelFactory);
        $this->factory->setEmailAttachmentEntityFactory($this->emailAttachmentEntityFactory);
    ...
    public function testCreateEmailModelWithNullAttachmentModelFactory(): void
    {
        $recipient = (new User())->setEmail('recipient1@example.com');
        $templateName = 'sample_template';
        $templateParams = ['sample_key' => 'sample_value'];
        ...
        $factory = new EmailModelFromEmailTemplateFactory(
            $this->emailTemplateContextProvider,
            $this->renderedEmailTemplateProvider,
            $this->createMock(EmailModelFactory::class),
            $this->emailOriginHelper,
            $this->entityOwnerAccessor
        );
        $factory->setEmailAttachmentModelFactory(null);
        $factory->setEmailAttachmentEntityFactory($this->emailAttachmentEntityFactory);

        $emailModelFactory = $this->createMock(EmailModelFactory::class);
        $emailModelFactory->expects(self::once())
            ->method('getEmail')
            ->willReturn(new EmailModel());
        ...
        // Attachment factories should not be called when one is null
        $this->emailAttachmentEntityFactory->expects(self::never())
            ->method('createEmailAttachmentEntities');

        $emailModel = $factory->createEmailModel($this->from, $recipient, $templateName, $templateParams);

        // No attachments should be processed due to BC layer
        self::assertCount(0, $emailModel->getAttachments());
        self::assertEquals($this->from->toString(), $emailModel->getFrom());
        self::assertEquals($emailOrigin, $emailModel->getOrigin());
    }

    public function testCreateEmailModelWithNullAttachmentEntityFactory(): void
    {
        $recipient = (new User())->setEmail('recipient1@example.com');
        $templateName = 'sample_template';
        $templateParams = ['sample_key' => 'sample_value'];
        ...
        $factory->setEmailAttachmentModelFactory($this->emailAttachmentModelFactory);
        $factory->setEmailAttachmentEntityFactory(null);
        ...
        // Attachment factories should not be called when one is null
        $this->emailAttachmentModelFactory->expects(self::never())
            ->method('createEmailAttachmentModels');
    ...
    public function testCreateEmailModelWithBothAttachmentFactoriesNull(): void
    {
        $recipient = (new User())->setEmail('recipient1@example.com');
        $templateName = 'sample_template';
        $templateParams = ['sample_key' => 'sample_value'];
        ...
        // Both factories are null by default, but explicitly set them to ensure clarity
        $factory->setEmailAttachmentModelFactory(null);
        $factory->setEmailAttachmentEntityFactory(null);
        ...
        // No attachments should be processed when both factories are null
        self::assertCount(0, $emailModel->getAttachments());
        self::assertEquals($this->from->toString(), $emailModel->getFrom());
        self::assertEquals($emailOrigin, $emailModel->getOrigin());
        self::assertEquals([$recipient->getEmail()], $emailModel->getTo());
