...
use Oro\Bundle\EmailBundle\EventListener\AvailableInTemplateEntityConfigListener;
...
class AvailableInTemplateEntityConfigListenerTest extends TestCase
{
    private ClearableConfigCacheInterface&MockObject $clearableConfigCache;
    private AvailableInTemplateEntityConfigListener $listener;

    #[\Override]
    protected function setUp(): void
    {
        $this->clearableConfigCache = $this->createMock(ClearableConfigCacheInterface::class);

        $this->listener = new AvailableInTemplateEntityConfigListener($this->clearableConfigCache);
    }
    ...
    public function testPreFlush(
        ...
        if ($shouldClearCache) {
            $this->clearableConfigCache->expects(self::once())
                ->method('clearCache');
        } else {
            $this->clearableConfigCache->expects(self::never())
                ->method('clearCache');
        }
