...
class SafeGetAttrNodeVisitorTest extends TestCase
{
    ...
    private GetAttrExpression $getAttrNode;
    private SafeGetAttrNode $safeGetAttrNode;
    ...
    protected function setUp(): void
    {
        ...
        $this->getAttrNode = new GetAttrExpression(
            new ConstantExpression('obj', 1),
            new ConstantExpression('field', 1),
            null,
            Template::ANY_CALL,
            1
        );
    ...
    /**
     * SafeGetAttrNode is itself a subclass of GetAttrExpression and must not be replaced again
     * to avoid infinite loops and double-wrapping.
     */
    public function testEnterNodeDoesNotReplaceSubclassOfGetAttrExpression(): void
    {
        self::assertSame($this->safeGetAttrNode, $this->visitor->enterNode($this->safeGetAttrNode, $this->env));
    }

    public function testEnterNodeReplacesExactGetAttrExpressionWithSafeGetAttrNode(): void
    {
        $result = $this->visitor->enterNode($this->getAttrNode, $this->env);

        self::assertInstanceOf(SafeGetAttrNode::class, $result);
        self::assertSame(SafeGetAttrNode::class, get_class($result));
    }
    ...
    public function testEnterNodeCopiesNodesAndAttributes(): void
    {
        ...
        $argumentsNode = new ArrayExpression([], 1);

        $node = new GetAttrExpression($objectNode, $attributeNode, $argumentsNode, Template::METHOD_CALL, 42);
        ...
        self::assertFalse($result->getAttribute('ignore_strict_check'));
        self::assertTrue($result->getAttribute('optimizable'));
