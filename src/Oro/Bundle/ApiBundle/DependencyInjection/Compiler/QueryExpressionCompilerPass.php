...
class QueryExpressionCompilerPass implements CompilerPassInterface
{
    ...
    const COMPOSITE_OPERATOR_PLACEHOLDER = 'expressionType';

    const COMPARISON_EXPRESSION_TAG         = 'oro.api.query.comparison_expression';
    const COMPARISON_OPERATOR_PLACEHOLDER = 'operator';
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        $visitorServiceDef->setArguments([
            $this->getExpressions($container, self::COMPOSITE_EXPRESSION_TAG, self::COMPOSITE_OPERATOR_PLACEHOLDER),
            $this->getExpressions($container, self::COMPARISON_EXPRESSION_TAG, self::COMPARISON_OPERATOR_PLACEHOLDER)
        ]);
    }
