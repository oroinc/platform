...
class OroApiExtension extends Extension implements PrependExtensionInterface
{
    ...
    private function registerConfigParameters(ContainerBuilder $container, array $config): void
    {
        ...
        $container->setParameter('oro_api.use_absolute_urls_for_api', $config['use_absolute_urls_for_api']);
        $container->setParameter('oro_api.ext_id_entities', $config['ext_id_entities']);
    ...
    private function registerFilterDisallowDataTypes(ContainerBuilder $container, array $config): void
    {
        $container->getDefinition('oro_api.get_config.complete_filters')
            ->replaceArgument(2, $config['filter_disallow_array_data_types'])
            ->replaceArgument(3, $config['filter_disallow_range_data_types']);
    }
