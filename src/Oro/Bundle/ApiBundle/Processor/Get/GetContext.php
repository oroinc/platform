...
class GetContext extends SingleItemContext
{
    ...
    protected function createActionMetadataExtra(string $action): ActionMetadataExtra
    {
        $parentAction = $this->getParentAction();
        if (!$parentAction) {
            return parent::createActionMetadataExtra($action);
        }

        $actionMetadataExtra = new ActionMetadataExtra($action);
        $actionMetadataExtra->setParentAction($parentAction);

        return $actionMetadataExtra;
    }
}
