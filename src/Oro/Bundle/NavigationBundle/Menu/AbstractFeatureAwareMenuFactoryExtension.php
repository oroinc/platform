...
abstract class AbstractFeatureAwareMenuFactoryExtension implements Factory\ExtensionInterface
{
    /**
     * @var FeatureChecker
     */
    ...
    public function buildOptions(array $options = [])
    {
        if (!$this->alreadyDenied($options) && !empty($options['route'])) {
            $options['extras']['isAllowed'] = $this->featureChecker
                ->isResourceEnabled($options['route'], 'route', $this->getScopeId());
        }
     ...
     */
    abstract protected function getScopeId();
}
