...
class ActivityAssociationRouteOptionsResolverTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testResolveActivityAssociationRouteWithoutEntityPlaceholder()
    {
        $route = new Route('/route', [], [], ['group' => 'activity_associations']);
    ...
    {
        $route = new Route('/{entity}/route', [], [], ['group' => 'activity_associations']);
        ...
        $this->assertEquals(['entity' => 'emails|calls|notes'], $route->getRequirements());

        $this->routeCollection->sortByPriority();
        $this->assertEquals(
            ['override1', 'override2', 'first_route', 'some_route', 'tested_route', 'last_route'],
