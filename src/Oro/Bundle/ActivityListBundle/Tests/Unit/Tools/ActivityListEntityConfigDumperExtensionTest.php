...
class ActivityListEntityConfigDumperExtensionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testEmptyTargetsSupports()
    {
        $provider = $this->getMockBuilder('Oro\Bundle\EntityConfigBundle\Provider\ConfigProvider')
            ->disableOriginalConstructor()->getMock();
        $provider->expects($this->once())->method('getConfigs')->willReturn([]);
        $this->configManager->expects($this->once())->method('getProvider')->willReturn($provider);
    ...
    public function testSupports()
    {
        ...
        $this->listProvider->expects($this->any())->method('getTargetEntityClasses')
            ->willReturn(['Acme\TestBundle\Entity\TestEntity']);
        $provider->expects($this->once())->method('hasConfig')
            ->with(ActivityListEntityConfigDumperExtension::ENTITY_CLASS)->willReturn(true);
    ...
    public function testPreUpdate()
    {
        ...
        $this->listProvider->expects($this->any())->method('getTargetEntityClasses')
            ->willReturn(['Acme\TestBundle\Entity\TestEntity']);
        $this->configManager->expects($this->any())->method('getProvider')->willReturn($provider);
        $this->associationBuilder->expects($this->once())->method('createManyToManyAssociation')
            ->with(
