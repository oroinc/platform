...
class ImageRemovalManagerTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $this->attachmentFileManager = $this->createMock(FileManager::class);
    ...
    {
        $file = new FileModel();
        $file->setFilename('orig.jpg');

        $this->attachmentFileManager->expects($this->once())
            ->method('deleteFile')
