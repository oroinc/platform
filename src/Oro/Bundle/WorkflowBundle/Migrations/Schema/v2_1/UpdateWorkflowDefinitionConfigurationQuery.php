...
class UpdateWorkflowDefinitionConfigurationQuery extends ParametrizedMigrationQuery
{
    ...
    protected function migrateConfigs(LoggerInterface $logger, $dryRun = false)
    {
        ...
        foreach ($rows as $key => $row) {
            ...
            if ($data && isset($data['transitions'])) {
                foreach ($data['transitions'] as $transition => &$option) {
                    if (isset($option['frontend_options']['icon'])) {
                        $newIconName = $this->createIconName($option['frontend_options']['icon']);

                        if ($newIconName !== $option['frontend_options']['icon']) {
                            $option['frontend_options']['icon'] = $newIconName;
                            $dataChanged = true;
                        }
                    }
                }
