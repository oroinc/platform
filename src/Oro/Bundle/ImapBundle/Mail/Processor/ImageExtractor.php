...
class ImageExtractor implements ContentIdExtractorInterface
{
    public function extract(PartInterface $multipart)
    {
        ...
        foreach ($multipart as $part) {
            ...
            if ($this->supports($part)) {
                ...
                $replacement = sprintf('data:%s;base64,%s', $headers->get('Content-Type')->getType(), $part->getContent());
    ...
    {
        return $part->getHeaders()->has('Content-Type') && strpos($part->getHeaders()->get('Content-Type')->getFieldValue(), 'image/') === 0;
    }
