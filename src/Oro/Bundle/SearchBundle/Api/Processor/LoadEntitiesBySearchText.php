...
class LoadEntitiesBySearchText implements ProcessorInterface
{
    public function __construct(
        ...
        private readonly SearchEntityListFilterHelper $searchEntityListFilterHelper,
    ...
    private function processSearchQuery(Criteria $criteria, array $entities, ListContext $context): void
    {
        ...
        $toJoinAggregation = [];
        $aggregationFilterValue = $filterValues->getOne('aggregations');
    ...
    {
        $searchQuery = $filterValues->getOne('searchQuery')?->getValue() ?? '';
        $searchText = $filterValues->getOne('searchText')?->getValue();
        if ($searchText) {
            if ($searchQuery) {
    ...
    {
        $searchQueryFilterValue = $filterValues->getOne('searchQuery');
