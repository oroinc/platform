...
class DeprecationErrorHandler
{
    ...
    const WEAK_MODE_PATTERN_DELIMITER = ':';

    private static $isRegistered = false;

    private static $weakVendorsPattern = null;
    ...
    public static function register($mode = 0)
    {
        ...
        $getMode = function () use ($mode) {
            ...
            if (strpos($mode, self::MODE_WEAK_VENDORS . self::WEAK_MODE_PATTERN_DELIMITER) === 0) {
                list($mode, $weakVendorsPattern) = explode(self::WEAK_MODE_PATTERN_DELIMITER, $mode, 2);
                self::$weakVendorsPattern = $weakVendorsPattern;
            }
        ...
        if (null !== $oldErrorHandler) {
            ...
            register_shutdown_function(function () use ($getMode, &$deprecations, $deprecationHandler, $colorize) {
                ...
                $displayDeprecations = function ($deprecations) use ($colorize, $cmp, $groups, $mode) {
                    foreach ($groups as $group) {
                        if ($deprecations[$group.'Count']) {
                            uasort($deprecations[$group], $cmp);

                            foreach ($deprecations[$group] as $msg => $notices) {
                                if ($mode === self::MODE_WEAK_VENDORS
                                    && self::$weakVendorsPattern
                                    && $group === 'remaining vendor') {
                                    break;
                                }
                ...
                register_shutdown_function(function () use (&$deprecations, $displayDeprecations, $mode) {
                    foreach ($deprecations as $group => $arrayOrInt) {
